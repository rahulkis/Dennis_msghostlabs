...
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/class-order-form-db.php';
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/class-order-form-setup-wizard.php';

// V2 Admin Notes
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/wc-admin-notes/class-wwof-setup-wizard-note.php';
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/wc-admin-notes/class-wwof-migration-wizard-note.php';

// V2 API
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/class-order-form-generator.php';
...
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/controllers/class-rest-api-order-form-api-keys-controller.php';
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/controllers/class-rest-api-i18n-controller.php';
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/controllers/class-rest-api-setup-wizard-controller.php';
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/controllers/class-rest-api-variations-controller.php';

// WWOF API Helpers
require_once WWOF_INCLUDES_ROOT_DIR . 'v2/api/helpers/class-order-form-api-helpers.php';
...
class WooCommerce_WholeSale_Order_Form
{
     ...
     * @since 2.0   Toggle settings between old and new form.
     * @since 2.0.2 Only show old settings if Use Legacy filter is used and if user is using old installation
     */
    public function wwof_plugin_settings($settings)
    {
        ...
        if ($use_legacy && !Order_Form_Helpers::is_fresh_install()) {
            ...
            // V2 Settings
            $settings[] = include WWOF_INCLUDES_ROOT_DIR . 'v2/class-order-form-settings.php';
    ...
    /**
     * Override use legacy form filter value.
     * - Return false to use the v2 settings else use old settings
     * - Return true to use old settings if wizard setup is not yet done
     *
     * @param $value
     *
     * @return array
     * @since 2.0
     */
    public function use_legacy_order_form_filter($value)
    {

        // If fresh install then use the v2 settings
        if (Order_Form_Helpers::is_fresh_install()) {
            return false;
        }

        // If v2 and below then use the old settings
        // Still use old settings if migration is not complete
        if (get_option(WWOF_WIZARD_SETUP_DONE, 'no') != 'yes') {
            return true;
        }
    ...
    public function run()
    {
        ...
        // Filter on what settings to display
        add_filter('wwof_use_legacy_order_form_filter', array($this, 'use_legacy_order_form_filter'));
