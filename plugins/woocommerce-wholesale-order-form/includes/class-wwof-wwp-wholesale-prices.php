...
if ( ! class_exists( 'WWOF_WWP_Wholesale_Prices' ) ) {
    ...
    class WWOF_WWP_Wholesale_Prices {
        ...
        public function wwof_display_wholesale_price_requirement() {
            ...
            $min_req_data = $this->get_min_requirement_data( $wholesale_role[0] );
            extract( $min_req_data );

            if ( ( ! empty( $min_order_quantity ) || ! empty( $min_order_price ) ) ) {
                $message = $this->wwof_get_wholesale_price_requirement_message(
                    $min_order_quantity,
                    $min_order_price,
                    $min_req_logic
                );
        ...
        public function get_min_requirement_data( $wholesale_role ) {
            ...
            if ( get_user_meta( $user_id, 'wwpp_override_min_order_qty', true ) == 'yes' ) {
                ...
                ) == 'yes' && isset( $wwpp_order_requirement_mapping[ $wholesale_role ] ) ) {
        ...
        public function wwof_get_product_shop_price_with_taxing_applied( $product, $price, $wc_price_arg = array() ) {
            ...
            if ( $taxes_enabled == 'yes' && $wholesale_tax_display_shop == 'incl' ) {
                $filtered_price = wc_price(
                    WWOF_Functions::wwof_get_price_including_tax( $product, array( 'qty' => 1, 'price' => $price ) )
                );
            } elseif ( $wholesale_tax_display_shop == 'excl' ) {
                $filtered_price = wc_price(
                    WWOF_Functions::wwof_get_price_excluding_tax( $product, array( 'qty' => 1, 'price' => $price ) ),
                    $wc_price_arg
                );
            } else {
            ...
            return apply_filters(
                'wwpp_filter_product_shop_price_with_taxing_applied',
                $filtered_price,
                $price,
                $product
            );
        ...
        public function wwof_get_product_price( $product ) {
            ...
            $price_html                  = "";
            $hide_wholesale_discount     = get_option(
                "wwof_general_hide_quantity_discounts"
            ); // Option to hide Product Quantity Based Wholesale Pricing

            do_action( 'before_wwof_get_product_price', $product );

            if ( WWOF_Functions::wwof_get_product_type( $product ) == 'simple' || WWOF_Functions::wwof_get_product_type(
                    $product
                ) == 'variation' ) {
        ...
        public function wwof_get_product_quantity_field( $product, $product_id, $product_type, $wholesale_role ) {
                ...
                if ( ! empty( $wholesale_role ) ) {
                    ...
                    if ( $product_type != 'variable' ) {
                        ...
                        if ( is_numeric( $wholesale_price ) ) {
                            ...
                            $min_order_qty = get_post_meta(
                                $product_id,
                                $wholesale_role[0] . '_wholesale_minimum_order_quantity',
                                true
                            );
                            if ( $min_order_qty ) {
            ...
            if ( $product->is_in_stock() ) {
                ...
                $quantity_field = str_replace(
                    'type="number"',
                    'type="number" tabindex="' . $tab_index_counter . '" input_value="' . $initial_value . '"',
                    $quantity_field
                );
            } else {
                $quantity_field = '<span class="out-of-stock">' . __(
                        'Out of Stock',
         ...
         * @since 1.6.6 Refactor codebase and move to its proper model
         */
        public function wwof_get_wholesale_price_requirement_message(
            $wholesale_min_order_quantity,
            $wholesale_min_order_price,
            $wholesale_min_req_logic
        ) {
            ...
            if ( WWOF_ACS_Integration_Helper::aelia_currency_switcher_active() ) {
                ...
                $shop_base_currency = WWOF_ACS_Integration_Helper::shop_base_currency();

                if ( $active_currency != $shop_base_currency ) {
                    $wholesale_min_order_price = WWOF_ACS_Integration_Helper::convert(
                        $wholesale_min_order_price,
                        $active_currency,
                        $shop_base_currency
                    );
            ...
            if ( ! empty( $wholesale_min_order_quantity ) && ! empty( $wholesale_min_order_price ) && ! empty( $wholesale_min_req_logic ) ) {
                $message = sprintf(
                    __(
                        '<b>NOTE:</b> A minimum order quantity of <b>%1$s</b> %2$s minimum order subtotal of <b>%3$s</b> is required to activate wholesale pricing in the cart.',
                        'woocommerce-wholesale-order-form'
                    ),
                    $wholesale_min_order_quantity,
                    $wholesale_min_req_logic,
                    $this->formatted_price( $wholesale_min_order_price )
                );
            } elseif ( ! empty( $wholesale_min_order_quantity ) ) {
                $message = sprintf(
                    __(
                        '<b>NOTE:</b> A minimum order quantity of <b>%1$s</b> is required to activate wholesale pricing in the cart.',
                        'woocommerce-wholesale-order-form'
                    ),
                    $wholesale_min_order_quantity
                );
                ...
                $message = sprintf(
                    __(
                        '<b>NOTE:</b> A minimum order subtotal of <b>%1$s</b> is required to activate wholesale pricing in the cart.',
                        'woocommerce-wholesale-order-form'
                    ),
                    $this->formatted_price( $wholesale_min_order_price )
                );
        ...
        public function formatted_price( $wholesale_min_order_price ) {
            ...
            if ( WWOF_Functions::is_plugin_active(
         ...
         * @since 1.3.1
         */
        private function wwof_get_specific_currency_mapping(
            $mapping,
            $user_wholesale_role,
            $active_currency,
            $base_currency_mapping
        ) {
        ...
        public function wwof_show_addon_sub_total( $price_html, $product ) {
            ...
            if ( $Product_Addon_Display != null && ( get_class(
                        $Product_Addon_Display
                    ) == 'WC_Product_Addons_Display' || get_class(
                        $Product_Addon_Display
                    ) == 'Product_Addon_Display_Legacy' ) ) {
                ...
                if ( is_object( $the_product ) ) {
                    ...
                    $display_price    = 'incl' === $tax_display_mode ? wc_get_price_including_tax(
                        $the_product
                ...
                $display_totals = '<div class="product-addons-total" data-show-sub-total="' . ( apply_filters(
                        'woocommerce_product_addons_show_grand_total',
                        true,
                        $the_product
                    ) ? 1 : 0 ) . '" data-type="' . esc_attr(
                        $the_product->get_type()
                    ) . '" data-tax-mode="' . esc_attr( $tax_mode ) . '" data-tax-display-mode="' . esc_attr(
                        $tax_display_mode
                    ) . '" data-price="' . esc_attr( $display_price ) . '" data-raw-price="' . esc_attr(
                        $raw_price
                    ) . '" data-product-id="' . esc_attr( $post_id ) . '"></div>';
                ...
                $results = $wpdb->get_results(
                    "SELECT p.ID FROM $wpdb->posts p
                                                ...
                                                WHERE p.post_status = 'publish'
                                                    ...
                                                    AND tr.term_taxonomy_id IN ( " . implode(
                        ',',
                        $filtered_terms_ids
                    ) . " )",
                    ARRAY_A
        ...
        public function get_all_wholesale_products( $role ) {
            ...
            if ( $role ) {
                ...
                if ( $wholesale_products && get_option(
                        'wwof_general_list_product_variation_individually',
                        'no'
                    ) === 'yes' ) {
                    $results2 = $wpdb->get_results(
                        "SELECT p.ID FROM $wpdb->posts p
            ...
            if ( $terms ) {
                ...
                foreach ( $terms as $term_id ) {
                    ...
                    if ( $term_meta[ $role . '_wholesale_discount' ] ) {
                        ...
                        $wholesale_products_via_cat = array_unique(
                            array_merge( $wholesale_products_via_cat, $wholesale_products, $wholesale_variations )
                        );
                    }
                }
        ...
        public function wwof_pre_get_posts_arg( $query_args ) {
            ...
            if ( class_exists( 'WooCommerceWholeSalePrices' ) && class_exists( 'WooCommerceWholeSalePricesPremium' ) ) {
                ...
                if ( get_option( 'wwof_general_list_product_variation_individually', 'no' ) === 'yes' ) {
                    ...
                    $user_wholesale_role        = ( is_array(
                            $user_wholesale_role
                        ) && ! empty( $user_wholesale_role ) ) ? $user_wholesale_role[0] : '';
                    $wholesale_role_discount    = get_option(
                        WWPP_OPTION_WHOLESALE_ROLE_GENERAL_DISCOUNT_MAPPING,
                        array()
                    );
                    $override_discount_per_user = get_user_meta(
                        get_current_user_id(),
                        'wwpp_override_wholesale_discount',
                        true
                    );
                    ...
                    if ( ! isset( $wholesale_role_discount[ $user_wholesale_role ] ) ) {
                        ...
                        $wwpp_settings_only_show_wholesale_products_to_wholesale_users = get_option(
                        ...
                        if ( $override_discount_per_user != 'yes' && ! current_user_can(
                                'manage_options'
                            ) && ! current_user_can( 'manage_woocommerce' ) && $user_wholesale_role ) {
                            ...
                            if ( $wwpp_settings_only_show_wholesale_products_to_wholesale_users === 'yes' ) {
                                ...
                                $wholesale_products_via_cat = $this->get_all_wholesale_products_from_category(
                                    $user_wholesale_role
                                );
                                $merged_wholesale_products  = array_unique(
                                    array_merge( $wholesale_products, $wholesale_products_via_cat )
                                );

                                $query_args['post__in'] = ! empty( $merged_wholesale_products ) ? array_intersect(
                                    $query_args['post__in'],
                                    $merged_wholesale_products
                                ) : array();
                            }
                        }
                        ...
                        if ( ! empty( $restricted_products ) ) {
                            ...
                            $query_args['post__not_in'] = array_intersect(
                        ...
                        if ( isset( $query_args['searched_keyword'] ) ) {
                            ...
                            $search_products = WWOF_Product_Listing_Helper::get_search_products(
                                $query_args['searched_keyword'],
                                $search_the_sku
                            );
        ...
        public function wwof_remove_wwpp_pre_get_posts_arg_hook() {
            ...
            if ( class_exists( 'WooCommerceWholeSalePrices' ) && class_exists( 'WooCommerceWholeSalePricesPremium' ) ) {
                ...
                remove_filter(
                    'wwof_filter_product_listing_query_arg',
                    array( $wc_wholesale_prices_premium->wwpp_query, 'pre_get_posts_arg' )
                );
            }
        }
        ...
        public function wwof_product_query_args( $args, $filters ) {
            ...
            // WWP and WWPP
            $wwpp_override_discount_per_user = get_user_meta(
                get_current_user_id(),
                'wwpp_override_wholesale_discount',
                true
            );
            ...
            // If user is not admin/shop manager. Admin should see all products.
            if ( ! current_user_can( 'manage_woocommerce' ) ) {
                ...
                if ( ! empty( $user_wholesale_role ) && $only_show_wholesale_products && ! $this->_wholesale_user_have_general_role_discount(
                        $user_wholesale_role
                    ) ) {
                    ...
                    } elseif ( ! empty( $args['post__in'] ) && ! in_array( '0', $args['post__in'] ) ) {

                        $args['post__in']   = $this->filter_products(
                            $args['post__in'],
                            $args,
                            $user_wholesale_role,
                            $only_show_wholesale_products,
                            $wwpp_override_discount_per_user
                        );
                        $args['meta_query'] = array();
                        $args['tax_query']  = array();
                    }
            ...
            } else { // If user is Administrator or shop managers

                // If there are defined products in the shortcode attributes then we use it
                // give all access to products for admin
                if ( empty( $args['post__in'] ) && empty( $wwpp_products ) ) {
        ...
        public function filter_products(
            ...
            $list_product_variation_individually = get_option(
                'wwof_general_list_product_variation_individually',
                'no'
            );
            ...
            // Set meta query
            if ( ! empty( $user_wholesale_role ) && ! empty( $restricted_products ) && $only_show_wholesale_products && $wwpp_override_discount_per_user !== 'yes' ) {
            ...
            if ( ! empty( $user_wholesale_role ) && ! empty(
                $this->_product_cat_wholesale_role_filter()
            ...
            if ( $list_product_variation_individually == 'yes' && ! empty( $restricted_products_filtered ) ) {
                ...
                // Wholesale Customers and "Only Show Wholesale Products To Wholesale Customers" is enabled
                if ( ( ! empty( $user_wholesale_role ) && $only_show_wholesale_products ) ) {
                    ...
                    $variations = $wpdb->get_results(
                        ...
                        WHERE p.post_status = 'publish'
                            ...
                            AND p.post_parent IN ( " . implode(
                            ',',
                            array_map( 'intval', $restricted_products_filtered )
                        ) . " )",
                        ARRAY_A
                    );

                    $variations_from_category = $wpdb->get_results(
                        "SELECT p.ID, p.post_parent FROM $wpdb->posts p
                            INNER JOIN $wpdb->postmeta pm1 ON ( p.ID = pm1.post_id )
                            WHERE p.post_status = 'publish'
                                AND p.post_type = 'product_variation'
                                ...
                                AND p.post_parent IN (
                                    ...
                                    WHERE meta_key = '" . $user_wholesale_role . "_have_wholesale_price_set_by_product_cat'
                                        AND meta_value ='yes'
                                    AND $wpdb->posts.ID IN(" . implode(
                            ',',
                            array_map(
                                'intval',
                                $restricted_products_filtered
                            )
                        ) . ")
                                )",
                        ARRAY_A
                    ...
                    $variations = array_merge( $variations, $variations_from_category );
                } else {
                ...
                if ( ! empty( $variations ) ) {
                    ...
                    if ( current_user_can(
                            'manage_woocommerce'
                        ) || empty( $user_wholesale_role ) || ( is_user_logged_in(
                            ) && ! empty( $user_wholesale_role ) ) ) {

                        foreach ( $variations as $variation ) {
                            if ( ! WWOF_Functions::is_wwpp_active() || current_user_can( 'manage_woocommerce' ) ) {
                                $restricted_variations[] = $variation['ID'];
                                $variable_products[]     = $variation['post_parent'];
                            } else {

                                $role_variation_is_visible = get_post_meta(
                                    $variation['ID'],
                                    'wwpp_product_wholesale_visibility_filter'
                                );

                                if ( empty( $role_variation_is_visible ) || in_array(
                                        'all',
                                        $role_variation_is_visible
                                    ) || in_array( $user_wholesale_role, $role_variation_is_visible ) ) {
                                    $restricted_variations[] = $variation['ID'];
                                }
                                $variable_products[] = $variation['post_parent'];
                            }
                ...
                $restricted_products_filtered = array_unique(
                    array_merge( $restricted_products_filtered, $restricted_variations )
                );
            }
        ...
        public function optimized_meta_query( $only_show_wholesale_products, $user_wholesale_role, $args ) {
            ...
            if ( $wwof_product_cache_option == 'yes' && ! empty( $wwof_cached_products_ids ) ) {
                ...
                // If cache is disabled / building cache
                ...
                return empty( $restricted_products ) ? array( 0 ) : $this->filter_products(
                    $restricted_products,
                    $args,
                    $user_wholesale_role,
                    $only_show_wholesale_products,
        ...
        private function _wholesale_user_have_general_role_discount( $user_wholesale_role ) {
            ...
            if ( class_exists( 'WooCommerceWholeSalePricesPremium' ) ) {
                ...
                $user_wholesale_discount = $wc_wholesale_prices_premium->wwpp_wholesale_price_wholesale_role->get_user_wholesale_role_level_discount(
                    get_current_user_id(),
                    $user_wholesale_role
                );
        ...
        private function _product_cat_wholesale_role_filter() {
            ...
            return class_exists( 'WooCommerceWholeSalePricesPremium' ) ? get_option(
                WWPP_OPTION_PRODUCT_CAT_WHOLESALE_ROLE_FILTER,
                array()
            ) : array();
        }
        ...
        public function _get_current_user_wholesale_role() {
            ...
            if ( class_exists( 'WooCommerceWholeSalePrices' ) ) {
                ...
                return ( is_array( $wholesale_role ) && ! empty( $wholesale_role ) ) ? array_values(
                    $wholesale_role
                ) : '';
            }
        ...
        /**
         * Removes filter woocommerce_get_children if shop manager login so that it will show all product variations without restrictions.
         *
         * @since  1.19.1
         * @access private
         */
        private function _remove_wwpp_woocommerce_get_children_filter() {

            if ( current_user_can( 'shop_manager' ) && WWOF_Functions::is_wwpp_active() ) {
                global $wc_wholesale_prices_premium;

                remove_filter(
                    'woocommerce_get_children',
                    array(
                        $wc_wholesale_prices_premium->wwpp_wholesale_price_variable_product,
                        'filter_available_variable_product_variations',
                    ),
                    10,
                    2
                );
        ...
        public function run() {
            ...
            // remove wwpp filter for woocommerce_get_children if WWPP is active
            $this->_remove_wwpp_woocommerce_get_children_filter();

            // Display wholesale price requirement message at the top of the search box wholesale ordering form.
            add_action(
                'wwof_action_before_product_listing_filter',
                array( $this, 'wwof_display_wholesale_price_requirement' ),
                10,
                1
            );
            ...
            add_filter(
                'wwof_display_wholesale_price_requirement',
                array( $this, 'wwof_show_hide_wholesale_price_requirement' ),
                10,
                1
            );
