    ...
    class WWOF_AJAX
    {
        ...
        public function wwof_display_product_listing($paged = 1, $search = null, $cat_filter = null, $shortcode_atts = null, $lang = 0)
        {
            ...
            if (get_option('wwof_general_list_product_variation_individually', 'no') === 'yes') {
                ...
                // issue-45 - Fix product Sorting issues when Show Variations Individually option is enabled
                if (in_array($sort_by, array('default', 'date', 'sku', 'menu_order'))) {

                    if ($sort_by === 'default') {
                        $products_id = array();
                        foreach ($product_query_ids as $key => $product_id) {
                            $product                 = wc_get_product($product_id);
                            $products_id[$key]['ID'] = $product_id;

                            $products_id[$key]['temp_id'] = $product->get_parent_id() ? $product->get_parent_id() : $product_id;
                        }
                        ...
                        if ($sort_order === 'asc') {
                            ...
                            usort($products_id, function ($a, $b) {
                                return $b['temp_id'] - $a['temp_id'];
                            });
                        }

                        $product_query_ids = array();
                        foreach ($products_id as $key => $item) {
                            array_push($product_query_ids, $item['ID']);
                        }

                    }
                    // If sorty by SKU then sort the product ids by sku first
                    elseif ($sort_by === 'sku') {

                        $products_sku = array();
                        foreach ($product_query_ids as $key => $product_id) {
                            $product                   = wc_get_product($product_id);
                            $products_sku[$key]['ID']  = $product_id;
                            $products_sku[$key]['sku'] = $product->get_sku();
                        }

                        usort($products_sku, function ($a, $b) {
                            return strcmp($a['sku'], $b['sku']);
                        });

                        $product_query_ids = array();
                        foreach ($products_sku as $key => $item) {
                            array_push($product_query_ids, $item['ID']);
                        }
                        ...
                        foreach ($product_query_ids as $key => $product_id) {
                            $product                   = wc_get_product($product_id);
                            $products_date[$key]['ID'] = $product_id;
                            if ($product->get_parent_id()) {
                                $parent_product                      = wc_get_product($product->get_parent_id());
                                $products_date[$key]['date_created'] = $parent_product->get_date_created()->getTimestamp();
                            } else {
                                $products_date[$key]['date_created'] = $product->get_date_created()->getTimestamp();
                            }
                        }

                        usort($products_date, function ($a, $b) {
                            $rdiff = $a['date_created'] - $b['date_created'];
                            if ($rdiff) {
                                return $rdiff;
                            }
                        ...
                        $product_query_ids = array();
                        foreach ($products_date as $key => $item) {
                            array_push($product_query_ids, $item['ID']);
                        }
                    }
                    // If sorty by Title first then sort the menu order for the same product title
                    elseif ($sort_by === 'menu_order') {
                        $products_menu_order = array();
                        foreach ($product_query_ids as $key => $product_id) {
                            $product                            = wc_get_product($product_id);
                            $products_menu_order[$key]['ID']    = $product_id;
                            $products_menu_order[$key]['title'] = $product->get_title();
                            if ($product->get_parent_id()) {
                                $parent_product                          = wc_get_product($product->get_parent_id());
                                $products_menu_order[$key]['menu_order'] = $parent_product->get_menu_order();
                            } else {
                                $products_menu_order[$key]['menu_order'] = $product->get_menu_order();
                            }
                        }

                        usort($products_menu_order, function ($a, $b) {
                        ...
                        $product_query_ids = array();
                        foreach ($products_menu_order as $key => $item) {
                            array_push($product_query_ids, $item['ID']);
                        }
                    }

                    $product_ids_sorted = array();
                    $used_parent        = array();
                    foreach ($product_query_ids as $key => $product_id) {
                        $product = wc_get_product($product_id);

                        // If product has parent or is variable product
                        if ($product->get_parent_id()) {
                            $parent_id = $product->get_parent_id();

                            // check if parent already sorted, if not then group the child products
                            if (!in_array($parent_id, $used_parent)) {
                                $parent_product = wc_get_product($parent_id);
                                foreach ($parent_product->get_children() as $child_id) {
                                    //$child_product = wc_get_product($child_id);
                                    $product_ids_sorted[] = $child_id;
                                }
                        ...
                        // If simple product
                            ...
                            $product_ids_sorted[] = $product_id;
                        }

                    }

                    $product_query_ids = $product_ids_sorted;
                    ...
                    switch ($sort_by) {
                        case 'default':
                            ...
                            break;
                        case 'date':
                            $args['orderby'] = 'post__in date';
                            break;
                        case 'sku':
                            $args['orderby'] = 'post__in meta_value';
                            break;
                        case 'menu_order':
                            $args['orderby'] = 'post__in menu_order title';
                            break;
                    }
        ...
        public function wwof_add_product_to_cart($product_type = null, $product_id = null, $variation_id = null, $quantity = null, $addon = null)
        {
            ...
            $response = apply_filters('wwof_ajax_add_to_cart_response', $response);
         ...
         *
         * @param array|null $products     List of products that will be added to cart.
         * @return bool
         *
         * @since 1.1.0
         * @since 1.6.6 Refactor codebase and move to its proper model
        ...
        public function wwof_add_products_to_cart($products = null)
        {
            ...
            foreach ($products as $product) {
                ...
                if (!isset($product['variationID']) && strcasecmp($product['productType'], 'variable') == 0) {
                    ...
                    $validate_quantity = apply_filters('wwof_quantity_validation', $validate_quantity, $product);
