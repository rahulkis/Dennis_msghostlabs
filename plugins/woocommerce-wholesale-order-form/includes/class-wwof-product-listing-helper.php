...
if (!class_exists('WWOF_Product_Listing_Helper')) {
    ...
    class WWOF_Product_Listing_Helper
    {
        ...
        public static function wwof_get_excluded_product_ids()
        {
            ...
            $cached_data = wp_cache_get('wwof_get_excluded_product_ids_non_persistent');
            ...
            wp_cache_set('wwof_get_excluded_product_ids_non_persistent', $exclude_products);
        ...
        public static function wwof_get_excluded_hidden_products()
        {
            ...
            $cached_data = wp_cache_get('wwof_get_excluded_hidden_products_non_persistent');
            ...
            wp_cache_set('wwof_get_excluded_hidden_products_non_persistent', $hidden_products);
            ...
            if (!empty($product_type) && is_a($product, 'WC_Product') && in_array($product_type, array('variation', 'variable'))) {
                ...
                switch ($product_type) {
                    ...
                    case 'variable':
                        ...
                        $variation_id      = isset($variations[0]) ? $variations[0] : '';
        ...
        public static function wwof_validate_product_selected_quantity($quantity, $product_id, $variation_id = null)
        {
            ...
            $min        = (isset($input_args['min_value']) && $input_args['min_value']) ? $input_args['min_value'] : 0;
            $max        = (isset($input_args['max_value']) && $input_args['max_value']) ? $input_args['max_value'] : '';
            $excess_qty = $quantity - $min;

            if ($quantity < $min || ($max && $quantity > $max) || $excess_qty % $step != 0) {
                return false;
            }
        ...
        public static function wwof_get_variations_wholesale_price(&$available_variations, $wholesale_role)
        {
            ...
            $only_show_wholsale_product = class_exists('WooCommerceWholeSalePricesPremium') && get_option('wwpp_settings_only_show_wholesale_products_to_wholesale_users', false) == 'yes' ? true : false;
            ...
            foreach ($available_variations as $key => $value) {
                ...
                if ($only_show_wholsale_product && empty($wholesalePrice)) {
        ...
        public static function wwof_get_available_variations($variable_product, $product_id, $wholesale_role)
        {
            ...
            $roles_variation_is_visible = '';

            // Remove viariations that is exclusive to wholesale role set in variation level.
            if ($available_variations && !empty($wholesale_role) && WWOF_Functions::is_wwpp_active()) {
                foreach ($available_variations as $key => $variation) {
                    $variation_id               = $variation['variation_id'];
                    $roles_variation_is_visible = get_post_meta($variation_id, WWPP_PRODUCT_WHOLESALE_VISIBILITY_FILTER);
                    if (!empty($roles_variation_is_visible) && !in_array('all', $roles_variation_is_visible) && !in_array($wholesale_role[0], $roles_variation_is_visible)) {
                        unset($available_variations[$key]);
                    }
                }
            }
