...
<div class="wwof-product-add-ons-container">
    ...
    <h3 class="wwof-product-add-ons-title"><?php _e('Product Add-ons', 'woocommerce-wholesale-order-form');?> <span
            class="dashicons dashicons-arrow-down-alt2"></span></h3>
...
<?php return ob_get_clean();
    ...
    public function wwof_get_product_row_action_fields($product, $alternate = false)
    {
        ...
        if (isset($_REQUEST['wwof_variable_disabled']) && $_REQUEST['wwof_variable_disabled'] == true) {
            ...
            // #36 Always Allow Backorders option does not work if we use dropdown to show the variations
            $wwpp_wholesale_backorders_allowed = false;
            // Make sure WWPP is active
            if (Order_Form_Helpers::is_wwpp_active()) {
                ...
                if ( !current_user_can( 'manage_options' ) )
                    if ( get_option( 'wwpp_settings_always_allow_backorders_to_wholesale_users' , false ) == 'yes' && !empty( $user_wholesale_role ) )
                        $wwpp_wholesale_backorders_allowed = true;
            }

            // If all variations are out of stock we show "Out of Stock" text
            if (WWOF_Product_Listing_Helper::wwof_out_of_stock_variations_check($product, $product_id) && !$wwpp_wholesale_backorders_allowed) {
        ...
        if (isset($_POST['cart_subtotal_tax']) || get_option('wwof_general_display_cart_subtotal') == 'yes') {
            ...
            if (WC()->cart->get_cart_contents_count()) {
                if ($cart_subtotal_tax == 'excl' || get_option('wwof_general_cart_subtotal_prices_display') == 'excl') {
                    ...
                    echo '<span class="sub_total including_tax">';
                    _e('Subtotal: ', 'woocommerce-wholesale-order-form');
                    echo wc_price(WC()->cart->cart_contents_total + WC()->cart->tax_total) . ' <small>' . WC()->countries->inc_tax_or_vat() . '</small>';
                    echo '</span>';
                }
            } else {
                echo '<span class="empty_cart">' . __('Cart Empty', 'woocommerce-wholesale-order-form') . '</span>';
            }
            echo '</div>';
