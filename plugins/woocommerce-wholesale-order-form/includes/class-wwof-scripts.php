...
if (!class_exists('WWOF_Scripts')) {
    ...
    class WWOF_Scripts
    {
        ...
        public function wwof_load_back_end_styles_and_scripts()
        {
            ...
            if (in_array($screen->id, array('woocommerce_page_wc-settings'))) {
                ...
                if (!isset($_GET['section']) || $_GET['section'] == '') {
                    ...
                    wp_localize_script('wwof_HelpSettings_js', 'wwof_settings_debug_var', array(
                        'success_force_fetch_update_data_txt' => __('Successfully re-fetched plugin update data', 'woocommerce-wholesale-order-form'),
                        'failed_force_fetch_update_data_txt'  => __('Failed to re-fetch plugin update data', 'woocommerce-wholesale-order-form'),
                    ));
        ...
        public function wwof_load_front_end_styles_and_scripts()
        {
            ...
            if (($post && isset($post->post_content) && has_shortcode($post->post_content, 'wwof_product_listing')) || $force_load_scripts_styles === true) {
                ...
                // Load the JS woocommerce quantity script
