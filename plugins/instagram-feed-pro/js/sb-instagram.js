			...
			this.revealNewImages = function() {
				...
				$self.find('.sbi_item.sbi_new').each(function (index) {
					...
					if ($self.find('.sbi_info .sbi_caption').length) {
						...
						var short_text = runes.substr(captionText, 0, text_limit);
						...
						if (captionText.length > text_limit) {
							...
							var noBRs = captionText.replace(/<br>/g, "    "),
								short_textnoBrs = short_text.replace(/<br>/g, "    "),
								short_textPlusOne = noBRs.substring(0, text_limit + 1),
								short_textPlusOneLastChar = short_textPlusOne.charAt(short_textPlusOne.length - 1),
								lastWhiteSpace = short_textnoBrs.lastIndexOf(' ');
