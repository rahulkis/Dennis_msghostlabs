 ...
 *
 * @class    WC_GC_Background_Updater
 * @version  1.0.0
 */
class WC_GC_Background_Updater extends WP_Background_Process {
	...
	protected function task( $callback ) {
		...
		include_once  WC_GC_ABSPATH . 'includes/wc-gc-update-functions.php' ;

		if ( is_callable( $callback ) ) {

			WC_GC()->log( sprintf( '- Running %s callback...', $callback ), 'info', 'wc_gc_db_updates' );
			...
			WC_GC()->log( $message, 'info', 'wc_gc_db_updates' );

		} else {

			$result = false;

			WC_GC()->log( sprintf( '- Could not find %s callback.', $callback ), 'notice', 'wc_gc_db_updates' );
		}
