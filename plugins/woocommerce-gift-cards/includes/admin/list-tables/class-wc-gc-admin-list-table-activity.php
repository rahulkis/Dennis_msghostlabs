...
class WC_GC_Activity_List_Table extends WP_List_Table {
	...
	public function column_date( $item ) {
		...
		$t_time    = date_i18n( _x( 'Y/m/d g:i:s a', 'list table date hover format', 'woocommerce-gift-cards' ), $item[ 'date' ] );
		...
		if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS ) {
			...
			$h_time = date_i18n( _x( 'Y/m/d', 'list table date format', 'woocommerce-gift-cards' ), $item[ 'date' ] );
		}
