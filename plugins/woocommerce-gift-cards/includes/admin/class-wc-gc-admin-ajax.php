...
class WC_GC_Admin_Ajax {
		...
		if ( ! empty( $_POST[ 'fields' ] ) ) {
			...
			try {
				...
				if ( ! empty( $posted_form_fields[ 'wc_gc_giftcard_code' ] ) ) {
					...
					if ( ! preg_match( '/^([a-zA-Z0-9]{4}[\-]){3}[a-zA-Z0-9]{4}$/', sanitize_text_field( $posted_form_fields[ 'wc_gc_giftcard_code' ] ) ) ) {
						throw new Exception( __( 'Gift Card code must be 19 characters long and follow the format XXXX-XXXX-XXXX-XXXX, where X can be any letter or number.', 'woocommerce-gift-cards' ) );
					}
			...
			if ( $item->save() ) {
				$order->add_order_note( sprintf( __( '%s (#%d) configured.', 'woocommerce-gift-cards' ), $product->get_title(), $product->get_id() ), false, true );
			}
		}
	...
	public static function configure_order_item_gift_card() {
		...
		// Defaults.
		$to           = ! empty( $item->get_meta( 'wc_gc_giftcard_to', true ) ) ? sanitize_text_field( $item->get_meta( 'wc_gc_giftcard_to', true ) ) : '';
		$to           = empty( $to ) && ! empty( $item->get_meta( 'wc_gc_giftcard_to_multiple', true ) ) ? sanitize_text_field( $item->get_meta( 'wc_gc_giftcard_to_multiple', true ) ) : $to;
		$from         = ! empty( $item->get_meta( 'wc_gc_giftcard_from', true ) ) ? sanitize_text_field( wp_unslash( wptexturize( $item->get_meta( 'wc_gc_giftcard_from', true ) ) ) ) : ''; // @phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$deliver_date = ! empty( $item->get_meta( 'wc_gc_giftcard_delivery', true ) ) ? absint( $item->get_meta( 'wc_gc_giftcard_delivery', true ) ) : '';

		if ( $deliver_date && ! is_null( $item->get_meta( '_wc_gc_giftcard_delivery_gmt_offset', true ) ) ) {
			$deliver_date = wc_gc_convert_timestamp_to_gmt_offset( $deliver_date, -1 * (float) $item->get_meta( '_wc_gc_giftcard_delivery_gmt_offset', true ) );
		}
		$deliver_date = $deliver_date ? date_i18n( get_option( 'date_format' ), $deliver_date ) : '';

		$message      = ! empty( $item->get_meta( 'wc_gc_giftcard_message', true ) ) ? sanitize_textarea_field( str_replace( '<br />', "\n", wp_unslash( wptexturize( $item->get_meta( 'wc_gc_giftcard_message', true ) ) ) ) ) : ''; // @phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

		$code         = ! empty( $item->get_meta( 'wc_gc_giftcard_code', true ) ) ? sanitize_text_field( $item->get_meta( 'wc_gc_giftcard_code', true ) ) : ''; // @phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		try {
			...
			if ( ! empty( $order_item_ids ) ) {
				...
				foreach ( $order_item_ids as $item_id ) {
					...
					/* translators: %s gift card code. */
					$order->add_order_note( sprintf( __( 'Removed gift card: %s', 'woocommerce-gift-cards' ), $item->get_name() ), false, true );
