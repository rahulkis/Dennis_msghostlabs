...
class WC_GC_Admin {
	 ...
	 * @var string
	 */
	private static $bundled_selectsw_version = '1.1.2';
		...
		$content = '
			<div class="wp-suggested-text">' .
				...
				'<ul class="privacy-policy-tutorial">' .
					...
					'<li>' . __( 'Recipient e-mail.', 'woocommerce-gift-cards' ) . '</li>' .
	...
	public static function admin_resources() {
		...
		wp_register_script( 'wc-gc-writepanel', WC_GC()->get_plugin_url() . '/assets/js/admin/wc-gc-admin' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'wp-util', 'sw-admin-select-init' ), WC_GC()->get_plugin_version() );
		 ...
		 */
		if ( WC_GC()->is_current_screen( array( 'shop_order', 'product' ) ) ) {
