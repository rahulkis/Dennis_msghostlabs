...
class WC_GC_Admin_Analytics {
	...
	*/
	...
	public static function filter_order_stats( $data, $order ) {
		...
		$order_stats_mods = apply_filters( 'woocommerce_gc_order_stats_data_modifications', array( 'purchases', 'payments' ) );
		...
		if ( in_array( 'payments', $order_stats_mods ) ) {
		...
		if ( in_array( 'purchases', $order_stats_mods ) ) {
	...
	public static function add_regenerate_order_stats_tool( $tools ) {
		...
		if ( WC_GC_Core_Compatibility::is_wc_admin_enabled() ) {
			...
			$tools[ 'gc_regenerate_order_stats' ] = array(
				...
				'desc'     => sprintf( __( 'Regenerates historical Revenue reports in WoCommerce Analytics to <a href="%s" target="_blank">account for prepaid gift cards correctly</a>.', 'woocommerce-gift-cards' ), WC_GC()->get_resource_url( 'faq-multi-prepaid-revenue' ) ),
