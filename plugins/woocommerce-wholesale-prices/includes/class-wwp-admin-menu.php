    ...
    class WWP_Admin_Menu {
        ...
        public function wholesale_settings() {
            if ( ! WWP_Helper_Functions::is_wwof_active() && ! WWP_Helper_Functions::is_wwlc_active() ) {
                wp_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=wwp_settings' ) );
                exit;
            }

            $output = '<div class="wrap">';

            $output .= '<h1>' . __( 'Settings', 'woocommerce-wholesale-prices' ) . '</h1>';
            $output .= '<br/>';
            $output .= '<p style="font-size:20px;"><a href="' . admin_url( 'admin.php?page=wc-settings&tab=wwp_settings' ) . '">' . __( 'Wholesale Prices Settings Page', 'woocommerce-wholesale-prices' ) . '</a></p>';

            if ( WWP_Helper_Functions::is_wwof_active() ) {
                $output .= '<p style="font-size:20px;"><a href="' . admin_url( 'admin.php?page=wc-settings&tab=wwof_settings' ) . '">' . __( 'Wholesale Order Form Settings Page', 'woocommerce-wholesale-prices' ) . '</a></p>';
            }

            if ( WWP_Helper_Functions::is_wwlc_active() ) {
                $output .= '<p style="font-size:20px;"><a href="' . admin_url( 'admin.php?page=wc-settings&tab=wwlc_settings' ) . '">' . __( 'Wholesale Lead Capture Settings Page', 'woocommerce-wholesale-prices' ) . '</a></p>';
            }
