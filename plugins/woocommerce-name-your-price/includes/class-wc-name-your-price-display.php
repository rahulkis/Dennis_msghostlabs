...
class WC_Name_Your_Price_Display {
	...
	public function nyp_style() {
		if ( get_option( 'woocommerce_nyp_disable_css', 'no' ) !== 'yes' ) {
			$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
			wp_enqueue_style( 'woocommerce-nyp', WC_Name_Your_Price()->plugin_url() . '/assets/css/name-your-price' . $suffix . '.css', false, WC_Name_Your_Price()->version );
	...
	public function has_options( $has_options, $product ) {
		return ! $has_options && WC_Name_Your_Price_Helpers::is_nyp( $product ) ? true : $has_options;
	}
